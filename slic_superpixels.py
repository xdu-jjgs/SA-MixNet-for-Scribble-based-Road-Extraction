...
def slic(image, point, cut, filename, n_segments=100, compactness=10., max_iter=10, sigma=0,
    ...
    * `start_label` is introduced to handle the issue [4]_. The labels
    ...
    # print(np.shape(centroids), np.shape(point))
    vis = np.zeros((512, 512))
    final_point = np.zeros((512, 512))
    final_point[centroids[:, 1], centroids[:, 2]] = 255
    final_point = final_point - final_point * (cut / 255)
    x, y = np.where(final_point > 0)
    final_map = np.zeros(np.shape(x))
    final_map = np.vstack((final_map, x))
    final_map = np.vstack((final_map, y))
    final_map = (final_map.T).astype(int)
    vis[final_map[:, 1], final_map[:, 2]] = 128
    # print(np.shape(final_map))
    centroids = np.vstack((final_map, point))
    vis[point[:, 1], point[:, 2]] = 255
    # print(centroids)
