...
and https://github.com/anilbatra2185/road_connectivity
"""
import numpy as np
import random
...
from graph import get_keypoint
...
def superpixels_histograms_neighbors(img, superpix_dir, point, osm, filename):
    n_segments = 1024
    kernel = np.ones((int(512 / np.power(n_segments, 1 / 2)), int(512 / np.power(n_segments, 1 / 2))))
    cut_img = cv2.dilate(osm, kernel, 1)
    segments = slic(img, point=point, cut=cut_img, filename=filename, n_segments=n_segments, compactness=20)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_root', default='./data/DeepGlobe_tiny/train/sat/', type=str,
                        help='path to imagery')
    parser.add_argument('--osm_root', default='./data/DeepGlobe_tiny/train/osm/', type=str,
    ...
    parser.add_argument('--marking_root', default='./data/DeepGlobe_tiny/train/scribble/', type=str,
    ...
    parser.add_argument('--superpix_root', default='./data/DeepGlobe_tiny/train/superpix/', type=str,
    ...
    parser.add_argument('--pred_root', default='./data/DeepGlobe_tiny/train/pred/', type=str,
                        help='path to FCN prediction')
    parser.add_argument('--GSD', default=1, type=float,
                        help='meters per pixel')
    parser.add_argument('--a1', default=6, type=int,
                        help='the first buffer width (meters)')
    parser.add_argument('--a2', default=8, type=int,
                        help='the second buffer width (meters)')
    parser.add_argument('--output_graph_mask_root', default='./data/DeepGlobe_tiny/train/graph_mask/', type=str,
                        help='path to graph-based masks generated from Graph Construction')
    parser.add_argument('--output_buffer_mask_root', default='./data/DeepGlobe_tiny/train/buffer_mask/', type=str,
                        help='path to buffer-based masks generated from Buffer Inference')
    parser.add_argument('--output_proposal_mask_root', default='./data/DeepGlobe_tiny/train/proposal_mask/', type=str,
    ...
    already_list = os.listdir('./data/DeepGlobe_tiny/train/proposal_mask')
    # for already_name in  already_list:
        # print(already_name[:-4])
    for name in name_list:
        region_info = name[:-7]
        print(region_info)
        if (region_info + 'mask.png') not in already_list:
            ...
            if np.sum(osm) > 0:  # except for non-road
                point = get_keypoint(args.osm_root + region_info + 'osm.png', 512)
                #
                ...
                if args.is_save_superpix:
                    ...
                    superpix_dir = None
                # point is the selected keypoint
                ...
                graph_cut = do_graph_cut([fg_cumulative_hist, bg_cumulative_hist], [fg_segments, bg_segments],
                                         norm_hists, neighbors, bg_loss)
